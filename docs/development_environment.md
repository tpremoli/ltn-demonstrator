...
6. It is important that when developing a new, specific feature, make sure to place it in a branch. Branches should be relatively self-contained, and contain one main overarching feature or bug fix. To then apply your changes to the main branch, use pull requests, to avoid the main branch from getting clogged up.
7. Create a new branch: To create a new branch from the current branch, use the command: 
    - `git checkout -b {branch_name}`
This will create a new branch based on the branch that you’re currently in, meaning that the code will be based on whatever offshoot you’re in. When creating branches for new features, it is recommended to making sure you’re in the main branch before running this command. 
9. Update your local codebase: to make sure that your local branch files are synced with the github repo, use the following command: 
 - `git pull`
Beyond that, it can be helpful to “pull” the latest code from another branch, such as “main” to make sure that your git is synced properly before merging. The command is as follows: 
...
If this is a new branch, once all changes have been committed, you may need to publish it, which will appear in the source control panel: 
...
3. Select Branches and write PR: Essentially, you must define the branches to PR through. The default is to merge from your current branch into main (which is the main use of PRs) but you can change it to whatever your needs are.
...
6. Once you are done, switch back to either main or whatever branch you were working on. This is important, so you don’t accidentally modify another PR’s code.
