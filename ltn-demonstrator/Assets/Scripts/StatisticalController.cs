...
public class StatisticsManager : MonoBehaviour {
    ...
    private void Awake()
    {
        if (Instance == null)
        {
            ...
            StartCoroutine(checkForTerminationCoRoutine());
        }
        else
        {
    ...
    private IEnumerator<object> checkForTerminationCoRoutine() {
        while(true){
            TravellerManager tm = TravellerManager.Instance;
            if (tm==null){
                Debug.Log("coRoutine: TravellerManager null!");
                yield return new WaitForSeconds(5f);
                continue;
            }
            if (tm.FinishedFor(TERMINATION_CRITERIA))
            {
                Debug.Log("coRoutine: Endsim True!");
                endSim = true;
                yield break;
            }
            Debug.Log("coRoutine: Endsim False!");
